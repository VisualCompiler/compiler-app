const getTokenClass = (type: string) => {
  ...
  if (
    type === "PLUS" || type === "MINUS" || type === "MULTIPLY" ||
    type === "DIVIDE" || type === "ASSIGN" || type === "SEMICOLON" ||
    type === "LEFT_PAREN" || type === "RIGHT_PAREN" ||
    type === "LEFT_BRACK" || type === "RIGHT_BRACK"
  ) return "token-symbol";
...
export const TokenListContent = () => {
  ...
  const tokenLines = [
    ...
    },
    {
      line: 3,
      tokens: [
        { type: "RIGHT_BRACK", lexeme: "}", column: 1 },
      ]
    },
    {
      line: 4,
      tokens: [
        { type: "EOF", lexeme: "", column: 1 },
      ]
    }
  ];
  ...
  return (
    <div className="text-sm font-mono space-y-1 px-2">
      {tokenLines.map(({ line, tokens }) => (
        <div key={line}>
          <span className="text-muted-foreground pr-2 w-6 inline-block">{line}</span>
          {tokens.map((t, i) => (
            <span
              key={i}
              title={`Column ${t.column}`}
              className="mr-2"
            >
              <span className={getTokenClass(t.type)}>
                {t.type}
                (
